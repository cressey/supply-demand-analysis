...
public class EmployeeTabHostActivity extends TabActivity implements
	...
	public void onResume() {
		...
		nameText = (EditText) findViewById(R.id.employeeNameText);
	...
	public void onClick(View arg0) {
		...
		case R.id.saveButton:
			...
			if (name.equals("")) {
				...
				employee.setName(nameText.getText().toString());
				EmployeeTabHostActivity.this.finish();
			}
			break;
		case R.id.backButton:
			new AlertDialog.Builder(EmployeeTabHostActivity.this)
					.setTitle("Go Back")
					.setMessage(
	...
	private void setUpSkillsTab() {
		checkboxSpace = (TableLayout) findViewById(R.id.addEmployeeCheckboxLayout);
		TextView subj = (TextView) findViewById(R.id.addEmployeeSubj);
		TextView skillGroup = (TextView) findViewById(R.id.addEmployeeSkillGroup);
		TextView skill = (TextView) findViewById(R.id.addEmployeeSkills);
		...
		subjectSpinner = (Spinner) findViewById(R.id.employeeSubjectSpinner);
	...
	public void setUpMaterialsTab() {
		...
		selectedMaterials = employeeDao.getEmployeeMaterials(employee.getId());
	}
	...
	public void setUpContextsTab() {
		...
		selectedContexts = employeeDao.getEmployeeContexts(employee.getId());
	}
	...
	private void updateSelectedItem(DatabaseObject item, boolean selected,
		...
		if (item instanceof Skill) {
			if (selected) {
				employeeDao.addEmployeeSkill(employee, (Skill) item);
			} else {
				employeeDao.removeEmployeeSkill(employee, (Skill) item);
			}
		} else if (item instanceof Material) {
			if (selected) {
				employeeDao.addEmployeeMaterial(employee, (Material) item);
			} else {
				employeeDao.removeEmployeeMaterial(employee, (Material) item);
			}
		} else if (item instanceof SkillContext) {
			if (selected) {
				employeeDao.addEmployeeContext(employee, (SkillContext) item);
			} else {
				employeeDao
						.removeEmployeeContext(employee, (SkillContext) item);
			}
		}
	}
	...
	private void updateSkillGroupSpinner() {
		...
		skillGroupSpinner = (Spinner) findViewById(R.id.employeeSkillGroupSpinner);
	...
	public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
			long arg3) {
		if (arg0.getId() == R.id.employeeSubjectSpinner) {
			updateSkillGroupSpinner();
		} else if (arg0.getId() == R.id.employeeSkillGroupSpinner) {
