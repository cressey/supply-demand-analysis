...
public class ViewSkillsSheetActivity extends SuperActivity implements
		OnClickListener {
	...
	public void onResume() {
		super.onResume();
		setContentView(R.layout.viewskillsheet);
		setUpCoreButtonControls(this);
		int id = getIntent().getIntExtra("com.vjs.taskskillanalysis.id", 0);
		sheet = skillsSheetDao.getSkillsSheet(id);
		skills = skillsSheetDao.getSkillsSheetSkillsWithDetails(id);
		materials = skillsSheetDao.getSkillsSheetMaterialsWithDetails(id);
		contexts = skillsSheetDao.getSkillsSheetContextsWithDetails(id);
		TextView skillSheetName = (TextView) findViewById(R.id.viewSkillsSheetText);
		skillSheetName.setText(sheet.getName());
		skillDisplay = (LinearLayout) findViewById(R.id.viewSheetLinearLayout);
		...
		for (SkillDetails skillDetails : skills) {
			...
			if (!subject.equals(skillDetails.getSubjectName())) {
				...
				// and new group
				skillGroup = skillDetails.getSkillGroupName();
				groupText.setText(skillGroup);
				skillDisplay.addView(groupText);
				// and new skill
				skill = skillDetails.getName();
				skillText.setText(skill);
				skillDisplay.addView(skillText);
			} else if (!skillGroup.equals(skillDetails.getSkillGroupName())) {
				// new group
				skillGroup = skillDetails.getSkillGroupName();
				groupText.setText(skillGroup);
				skillDisplay.addView(groupText);
	...
	public void onClick(View arg0) {
		...
		case R.id.backButton:
			ViewSkillsSheetActivity.this.finish();
			break;
		case R.id.helpButton:
			String message = "Add a decent help message here please!";
			new AlertDialog.Builder(ViewSkillsSheetActivity.this)
					.setTitle("Go Back")
					.setMessage(message)
